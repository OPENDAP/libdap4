...
XDRFileUnMarshaller::XDRFileUnMarshaller(FILE *out) : _source(0) { _source = new_xdrstdio(out, XDR_DECODE); }

XDRFileUnMarshaller::XDRFileUnMarshaller() : UnMarshaller(), _source(0) {
    throw InternalErr(__FILE__, __LINE__, "Default constructor not implemented.");
}

XDRFileUnMarshaller::XDRFileUnMarshaller(const XDRFileUnMarshaller &um) : UnMarshaller(um), _source(0) {
    throw InternalErr(__FILE__, __LINE__, "Copy constructor not implemented.");
}

XDRFileUnMarshaller &XDRFileUnMarshaller::operator=(const XDRFileUnMarshaller &) {
...
void XDRFileUnMarshaller::get_url(string &val) { get_str(val); }

void XDRFileUnMarshaller::get_opaque(char *val, unsigned int len) { xdr_opaque(_source, val, len); }

void XDRFileUnMarshaller::get_int(int &val) {
    if (!xdr_int(_source, &val))
        throw Error(
            "Network I/O Error(1). This may be due to a bug in libdap or a\nproblem with the network connection.");
}

void XDRFileUnMarshaller::get_vector(char **val, unsigned int &num, Vector &) {
    if (!xdr_bytes(_source, val, &num, DODS_MAX_ARRAY))
        throw Error("Network I/O error (1).");
}

void XDRFileUnMarshaller::get_vector(char **val, unsigned int &num, int width, Vector &vec) {
    BaseType *var = vec.var();

    if (!xdr_array(_source, val, &num, DODS_MAX_ARRAY, width, XDRUtils::xdr_coder(var->type()))) {
        throw Error("Network I/O error (2).");
    }
}

void XDRFileUnMarshaller::dump(ostream &strm) const {
    strm << DapIndent::LMarg << "XDRFileUnMarshaller::dump - (" << (void *)this << ")" << endl;
}
