...
/** This function returns the ObjectType value that matches the given string.
    ...
    @deprecated  */
ObjectType get_type(const string &value) { return get_description_type(value); }
...
void set_mime_multipart(ostream &strm, const string &boundary, const string &start, ObjectType type,
                        const string &version, EncodingType enc, const time_t last_modified) {
    strm << "HTTP/1.0 200 OK" << CRLF;
    if (version == "") {
        strm << "XDODS-Server: " << DVR << CRLF;
        strm << "XOPeNDAP-Server: " << DVR << CRLF;
...
void set_mime_ddx_boundary(ostream &strm, const string &boundary, const string &cid, ObjectType type,
                           EncodingType enc) {
    strm << "--" << boundary << CRLF;
...
void set_mime_data_boundary(ostream &strm, const string &boundary, const string &cid, ObjectType type,
                            EncodingType enc) {
    strm << "--" << boundary << CRLF;
    strm << "Content-Type: application/octet-stream" << CRLF;
    strm << "Content-Id: <" << cid << ">" << CRLF;
...
string get_next_mime_header(FILE *in) {
    ...
    while (!feof(in)) {
        if (fgets(line, line_length, in) && (strncmp(line, CRLF, 2) == 0 || line[0] == '\n'))
            return "";
        else {
...
string read_multipart_boundary(FILE *in, const string &boundary) {
    ...
    if ((!boundary.empty() && is_boundary(boundary_line.c_str(), boundary)) || boundary_line.find("--") != 0)
...
void read_multipart_headers(FILE *in, const string &content_type, const ObjectType object_type, const string &cid) {
    ...
    if (!(ct && cd && ci))
        throw Error(internal_error, "The DAP4 data response document is broken - missing header.");
}
...
void read_multipart_headers(istream &in, const string &content_type, const ObjectType object_type, const string &cid) {
    ...
    while (!header.empty()) {
        ...
        if (name == "content-type") {
            ...
            cd = true;
            if (get_description_type(value) != object_type)
                throw Error("Content-Description '" + value +
