...
/** This class contains common functionality for the filter program used to
  ...
  information to stdout to send a response back to the user's program. 
  @author jhrg 8/26/97 */
...
public:
    ...
    /** Class invariant. 
      @return True is the class state is OK, false otherwise. */
    bool OK();
    ...
    /** Print usage information for a filter program. This is sent to stderr
      and will up in the httpd log file. In addition, an error object is sent
      back to the client program telling them that the server is hosed. */
    void print_usage();
