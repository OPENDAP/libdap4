...
static char rcsid[] not_used =
    {"$Id: Dap4Dataset.cc 21699 2009-11-05 00:06:01Z jimg $"
    };
...
// Glue for the Dap4Dataset parser defined in dds.lex
...
void
Dap4Dataset::duplicate(const Dap4Dataset &dds)
{
    DBG(cerr << "Entering Dap4Dataset::duplicate... " <<endl);
    name = dds.name;
    ...
    Dap4Dataset &dds_tmp = const_cast<Dap4Dataset &>(dds);
...
/** Make a Dap4Dataset which uses the given BaseTypeFactory to create variables.
    ...
    set_dataset_name(). */
Dap4Dataset::Dap4Dataset(BaseTypeFactory *factory, const string &n)

        : d_factory(factory), name(n), d_container(0), d_dap_major(2),
        d_dap_minor(0),
        d_request_xml_base(""), d_timeout(0)
{
    DBG(cerr << "Building a Dap4Dataset with client major/minor: "
            << d_dap_major << "." << d_dap_minor << endl);
}

/** The Dap4Dataset copy constructor. */
Dap4Dataset::Dap4Dataset(const Dap4Dataset &rhs) : DapObj()
{
    DBG(cerr << "Entering Dap4Dataset(const Dap4Dataset &rhs) ..." << endl);
    duplicate(rhs);
    DBG(cerr << " bye." << endl);
}
...
{
    // delete all the variables in this Dap4Dataset
...
{
    DBG(cerr << "Entering Dap4Dataset::operator= ..." << endl);
    if (this == &rhs)
        return *this;
...
/** @brief Look for the parent of an HDF4 dimension attribute
    ...
    was found. */
BaseType *
Dap4Dataset::find_hdf4_dimension_attribute_home(AttrTable::entry *source)
{
    BaseType *btp;
...
/** Given a DAS object, scavenge attributes from it and load them into this
    ...
    client) can easily add attributes directly using the Dap4Dataset::get_attr_table
    ...
    transition to Dap4Dataset objects which contain attribute information for the
    existing servers (Since they all make DAS objects separately from the
    Dap4Dataset). They could be modified to use the same AttrTable methods but
    operate on the AttrTable instances in a Dap4Dataset/BaseType instead of those in
    a DAS.

    @param das Get attribute information from this DAS. */
void
Dap4Dataset::transfer_attributes(DAS *das)
{
    // If there is a container set in the Dap4Dataset then get the container from
...
string
Dap4Dataset::get_dataset_name() const
{
    return name;
}
...
void
Dap4Dataset::set_dataset_name(const string &n)
{
    name = n;
}
...
void
Dap4Dataset::filename(const string &fn)
{
    d_filename = fn;
}
//@}
...
/** Given the dap protocol version either from a MIME header or from within
    the DDX Dataset element, parse that string and set the Dap4Dataset fields.
    @see set_dap_client_version()
    @param version_string The version string from the MIME of XML document.
 */
void
Dap4Dataset::set_dap_version(const string &version_string)
{
...
/** Get and set the current container. If there are multiple files being
    used to build this Dap4Dataset, using a container will set a virtual structure
 ...
 * name. */
void
Dap4Dataset::container_name(const string &cn)
{
    // we want to search the Dap4Dataset for the top level structure with the given
	...
	if( !d_container )
	{
	    ...
	    // while_container is null will add the new structure to Dap4Dataset and
...
/** @brief Adds a copy of the variable to the Dap4Dataset.
    Using the ptr_duplicate() method, perform a deep copy on the variable
    \e bt and adds the result to this Dap4Dataset.
    @note The copy will not copy data values.
    @param bt Source variable. */
void
Dap4Dataset::add_var(BaseType *bt)
{
    if (!bt)
        throw InternalErr(__FILE__, __LINE__,
                          "Trying to add a BaseType object with a NULL pointer.");

    DBG2(cerr << "In Dap4Dataset::add_var(), bt's address is: " << bt << endl);

    BaseType *btp = bt->ptr_duplicate();
    DBG2(cerr << "In Dap4Dataset::add_var(), btp's address is: " << btp << endl);
    if( d_container )
    {
...
/** Remove the named variable from the Dap4Dataset. This method is not smart about
    looking up names. The variable must exist at the top level of the Dap4Dataset and
    must match \e exactly the name given.

    @note Invalidates any iterators that reference the contents of the Dap4Dataset.
    @param n The name of the variable to remove. */
void
Dap4Dataset::del_var(const string &n)
{
    if( d_container )
    {
...
void
Dap4Dataset::del_var(Vars_iter i)
{
    if (i != vars.end()) {
        BaseType *bt = *i ;
        vars.erase(i) ;
...
void
Dap4Dataset::del_var(Vars_iter i1, Vars_iter i2)
{
...
/** Search for for variable <i>n</i> as above but record all
    ...
    could not be found. */
BaseType *
Dap4Dataset::var(const string &n, BaseType::btp_stack &s)
{
    return var(n, &s);
}
/** @brief Find the variable with the given name.
    ...
    in the Dap4Dataset, but there is no requirement that it do so.
...
BaseType *
Dap4Dataset::leaf_match(const string &n, BaseType::btp_stack *s)
{
    DBG(cerr << "Dap4Dataset::leaf_match: Looking for " << n << endl);

    for (Vars_iter i = vars.begin(); i != vars.end(); i++) {
        BaseType *btp = *i;
        DBG(cerr << "Dap4Dataset::leaf_match: Looking for " << n << " in: " << btp->name() << endl);
...
BaseType *
Dap4Dataset::exact_match(const string &name, BaseType::btp_stack *s)
{
    for (Vars_iter i = vars.begin(); i != vars.end(); i++) {
        BaseType *btp = *i;
...
/** @brief Returns the first variable in the Dap4Dataset. */

Dap4Dataset::Vars_iter
Dap4Dataset::var_begin()
{
    return vars.begin();
}

Dap4Dataset::Vars_riter
Dap4Dataset::var_rbegin()
{
    return vars.rbegin();
}
...
Dap4Dataset::Vars_iter
Dap4Dataset::get_vars_iter(int i)
{
    return vars.begin() + i;
}
...
BaseType *
Dap4Dataset::get_var_index(int i)
{
    return *(vars.begin() + i);
}

/** @brief Returns the number of variables in the Dap4Dataset. */
int
Dap4Dataset::num_var()
{
    return vars.size();
...
int
Dap4Dataset::get_timeout()
{
    //  Has to effect under win32
    return d_timeout;
}

/** @brief Traverse Dap4Dataset, set Sequence leaf nodes. */
void
Dap4Dataset::tag_nested_sequences()
{
    for (Vars_iter i = vars.begin(); i != vars.end(); i++) {
...
/** @brief Parse a Dap4Dataset from a file with the given name. */
void
Dap4Dataset::parse(string fname)
{
...
/** @brief Parse a Dap4Dataset from a file indicated by the input file descriptor. */
void
Dap4Dataset::parse(int fd)
{
#ifdef WIN32
...
/** @brief Parse a Dap4Dataset from a file indicated by the input file descriptor.
    Read the persistent representation of a Dap4Dataset from the FILE *in, parse it
    and create a matching binary object.
    @param in Read the persistent Dap4Dataset from this FILE*.
    @exception InternalErr Thrown if \c in is null
    @exception Error Thrown if the parse fails. */
void
Dap4Dataset::parse(FILE *in)
{
    if (!in) {
...
#if FILE_METHODS
/** @brief Print the entire Dap4Dataset to the specified file. */
void
Dap4Dataset::print(FILE *out)
{
    fprintf(out, "Dataset {\n") ;
...
/** @brief Print the entire Dap4Dataset to the specified ostream. */
void
Dap4Dataset::print(ostream &out)
{
    out << "Dataset {\n" ;
...
#if FILE_METHODS
/** @brief Print a constrained Dap4Dataset to the specified file.
...
*/
void
Dap4Dataset::print_constrained(FILE *out)
{
    fprintf(out, "Dataset {\n") ;
...
*/
void
Dap4Dataset::print_constrained(ostream &out)
{
    out << "Dataset {\n" ;
...
void
Dap4Dataset::print_xml(FILE *out, bool constrained, const string &blob)
{
...
// Used by Dap4Dataset::send() when returning data from a function call.
...
*/
bool
Dap4Dataset::mark(const string &n, bool state)
{
    BaseType::btp_stack *s = new BaseType::btp_stack;

    DBG2(cerr << "Dap4Dataset::mark: Looking for " << n << endl);
    ...
    DBG2(cerr << "Dap4Dataset::mark: Set variable " << variable->name()
    ...
    while (!s->empty()) {
        ...
        DBG2(cerr << "Dap4Dataset::mark: Set variable " << s->top()->name()
        ...
        DBG2(cerr << "Dap4Dataset::mark: Parent variable " << parent_name << " (a " << parent_type << ")" << endl);
...
*/
void
Dap4Dataset::mark_all(bool state)
{
    for (Vars_iter i = vars.begin(); i != vars.end(); i++)
        (*i)->set_send_p(state);
}
