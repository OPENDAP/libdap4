...
Int16::Int16(const Int16 &copy_from) : BaseType(copy_from) { d_buf = copy_from.d_buf; }

BaseType *Int16::ptr_duplicate() { return new Int16(*this); }
...
bool Int16::serialize(ConstraintEvaluator &eval, DDS &dds, Marshaller &m, bool ce_eval) {
...
bool Int16::deserialize(UnMarshaller &um, DDS *, bool) {
    um.get_int16(d_buf);

    return false;
}

void Int16::compute_checksum(Crc32 &checksum) { checksum.AddData(reinterpret_cast<uint8_t *>(&d_buf), sizeof(d_buf)); }
 ...
 */
void Int16::serialize(D4StreamMarshaller &m, DMR &, /*ConstraintEvaluator &,*/ bool) {
    if (!read_p())
...
void Int16::deserialize(D4StreamUnMarshaller &um, DMR &) { um.get_int16(d_buf); }

unsigned int Int16::val2buf(void *val, bool) {
    // Jose Garcia
...
unsigned int Int16::buf2val(void **val) {
    // Jose Garcia
...
dods_int16 Int16::value() const { return d_buf; }

bool Int16::set_value(dods_int16 i) {
    d_buf = i;
    set_read_p(true);
...
void Int16::print_val(FILE *out, string space, bool print_decl_p, bool is_root_grp) {
    ostringstream oss;
    print_val(oss, space, print_decl_p, is_root_grp);
    fwrite(oss.str().data(), sizeof(char), oss.str().length(), out);
}

void Int16::print_val(ostream &out, string space, bool print_decl_p, bool is_root_grp) {
    if (print_decl_p) {
...
bool Int16::ops(BaseType *b, int op) {
 ...
 * @return void
 */
void Int16::dump(ostream &strm) const {
    strm << DapIndent::LMarg << "Int16::dump - (" << (void *)this << ")" << endl;
    DapIndent::Indent();
    BaseType::dump(strm);
