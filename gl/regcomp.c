...
# endif
    (BITSET_WORD_MAX
     >> (SBC_MAX % BITSET_WORD_BITS == 0
         ? 0 : BITSET_WORD_BITS - SBC_MAX % BITSET_WORD_BITS))
};
#endif
...
#endif
    size_t max_object_size = MAX(sizeof(struct re_state_table_entry),
    ...
    if (IS_EPSILON_NODE(dfa->nodes[node].type))
        for (i = 0; i < dfa->edests[node].nelem; ++i) {
            ...
            if (dfa->eclosures[edest].nelem == 0) {
                err =
                    calc_eclosure_iter(&eclosure_elem, dfa, edest, false);
                if (BE(err != REG_NOERROR, 0))
                    return err;
            } else
...
static int
 internal_function
peek_token_bracket(re_token_t * token, re_string_t * input,
                   reg_syntax_t syntax)
{
    unsigned char c;
    if (re_string_eoi(input)) {
    ...
    case OP_DUP_QUESTION:
        if (syntax & RE_CONTEXT_INVALID_OPS) {
            *err = REG_BADRPT;
            ...
            fetch_token(token, regexp, syntax);
            return parse_expression(regexp, preg, token, syntax, nest,
                                    err);
        }
        /* else fall through  */
    case OP_CLOSE_SUBEXP:
...
# ifdef RE_ENABLE_I18N
    {
        ...
        if (mbcset) {
            ...
            if (BE(*range_alloc == mbcset->nranges, 0)) {
                ...
                if (BE
                    (new_array_start == NULL || new_array_end == NULL, 0))
                    return REG_ESPACE;
    ...
    bracket_elem_t *start_elem, *end_elem;
    {
        ...
        for (ch = 0; ch < SBC_MAX; ch++) {
            ...
            if (nrules == 0)
                ...
                ch_collseq =
                    __collseq_table_lookup(collseqwc, __btowc(ch));
...
# endif
    ...
    if (mbcset->nmbchars || mbcset->ncoll_syms || mbcset->nequiv_classes
        || mbcset->nranges || (dfa->mb_cur_max > 1
