...
int main(int argc, char *argv[])
{
    ...
    while ((option_char = getopt()) != -1)
        ...
        case 'h': {     // help - show test names
            cerr << "Usage: UInt64Test has the following tests:" << endl;
            const std::vector<Test*> &tests = libdap::UInt64Test::suite()->getTests();
            unsigned int prefix_len = libdap::UInt64Test::suite()->getName().append("::").length();
    ...
    if (i == argc) {
        ...
        for (; i < argc; ++i) {
            ...
            test = libdap::UInt64Test::suite()->getName().append("::").append(argv[i]);
