...
public:
    ...
    void cons_UInt16_test() {
        CPPUNIT_ASSERT(i1->value() == 0 && i1->dataset() == "b" && i1->name() == "a" && i1->type() == dods_uint16_c);
    ...
    void type_compare_test() {
        ...
        CPPUNIT_ASSERT_THROW(ui16.ops(0, SCAN_EQUAL), Error);
    }
...
int main(int argc, char *argv[]) { return run_tests<UInt16Test>(argc, argv) ? 0 : 1; }
