...
public:
    ...
    void cons_Int32_test() {
        CPPUNIT_ASSERT(i1->value() == 0 && i1->dataset() == "b" && i1->name() == "a" && i1->type() == dods_int32_c);
    ...
    void type_compare_test() {
        ...
        f32.set_value(42);
        f64.set_value(42);
        CPPUNIT_ASSERT(i32.value() == 42);
        CPPUNIT_ASSERT(i32.ops(&b1, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&b1, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&i8, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&i16, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&ui16, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&i32, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&ui32, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&i64, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&ui64, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&f32, SCAN_EQUAL));
        CPPUNIT_ASSERT(i32.d4_ops(&f64, SCAN_EQUAL));

        CPPUNIT_ASSERT_THROW(i32.d4_ops(&url, SCAN_EQUAL), Error);
        CPPUNIT_ASSERT_THROW(i32.d4_ops(&str, SCAN_EQUAL), Error);
        CPPUNIT_ASSERT_THROW(i32.d4_ops(&array, SCAN_EQUAL), Error);
        CPPUNIT_ASSERT_THROW(i32.ops(0, SCAN_EQUAL), Error);
    }
    ...
    void ops_exception_1_test() {
        ...
        Int32 i32 = Int32("a", "b");
        b1.set_read_p(false);
        CPPUNIT_ASSERT_THROW(i32.ops(&b1, SCAN_EQUAL), InternalErr);
    }

    void ops_exception_2_test() {
        Byte b1 = Byte("a");
        Int32 i32 = Int32("a", "b");
        i32.set_read_p(false);
        CPPUNIT_ASSERT_THROW(i32.ops(&b1, SCAN_EQUAL), InternalErr);
    }

    void dump_test() {
        ofstream ofs("Int32Test_dump.output", ios::trunc);
        i1->set_value(21);
        i1->dump(ofs);
        ofs.close();
        ifstream ifs("Int32Test_dump.output");
        while (!ifs.eof())
            ifs >> a;
        ifs.close();
    ...
    void print_test() {
        FILE *fp;
        CPPUNIT_ASSERT(fp = fopen("Int32Test.output", "w"));
        i1->set_value(22);
        i1->print_val(fp, " ", true);
        fclose(fp);
        ifstream ifs("Int32Test.output");
        while (!ifs.eof())
            ifs >> a;
        ifs.close();
...
int main(int argc, char *argv[]) { return run_tests<Int32Test>(argc, argv) ? 0 : 1; }
