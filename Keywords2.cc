 ...
 */
void Keywords::m_add_keyword(const keyword &word, const keyword_value &value) { d_parsed_keywords[word] = value; }
 ...
 */
bool Keywords::is_known_keyword(const string &word) const { return d_known_keywords.count(word) == 1; }
 ...
 */
bool Keywords::has_keyword(const keyword &kw) const { return d_parsed_keywords.count(kw) == 1; }
...
string Keywords::parse_keywords(const string &ce) {
    ...
    while (!projection.empty()) {
        ...
        string word, value;
        if (f_parse_keyword(next_word, word, value) && m_is_valid_keyword(word, value)) {
