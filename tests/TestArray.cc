...
void TestArray::_duplicate(const TestArray &ts) { d_series_values = ts.d_series_values; }

BaseType *TestArray::ptr_duplicate() { return new TestArray(*this); }

TestArray::TestArray(const string &n, BaseType *v, bool is_dap4) : Array(n, v, is_dap4), d_series_values(false) {}

TestArray::TestArray(const string &n, const string &d, BaseType *v, bool is_dap4)
    : Array(n, d, v, is_dap4), d_series_values(false) {}

TestArray::TestArray(const TestArray &rhs) : Array(rhs), TestCommon(rhs) { _duplicate(rhs); }
...
int TestArray::m_offset(int y, Dim_iter X, int x) { return y * dimension_size(X, false) + x; }
 ...
 */
template <typename T, class C> void TestArray::m_constrained_matrix(vector<T> &constrained_array) {
    ...
    DBG(cerr << "whole_array: "; copy(whole_array.begin(), whole_array.end(), ostream_iterator<T>(cerr, ", "));
        cerr << endl);
...
template <typename T> void TestArray::m_enum_constrained_matrix(vector<T> &constrained_array) {
    int unconstrained_size = 1;
    Dim_iter d = dim_begin();
    while (d != dim_end())
        unconstrained_size *= dimension_size(d++, false);
 ...
 */
template <typename T, class C> void TestArray::m_cardinal_type_read_helper() {
 ...
 * call to some data store. A private method.
 */
template <typename T> void TestArray::m_enum_type_read_helper() {
    if (get_series_values()) {
        if (dimensions() == 2) {
            vector<T> tmp(length());
