...
void TestD4Sequence::m_duplicate(const TestD4Sequence &ts) {
    d_current = ts.d_current;
    d_len = ts.d_len;
    d_series_values = ts.d_series_values;
}

BaseType *TestD4Sequence::ptr_duplicate() { return new TestD4Sequence(*this); }

TestD4Sequence::TestD4Sequence(const string &n) : D4Sequence(n), d_len(4), d_current(0), d_series_values(false) {}

TestD4Sequence::TestD4Sequence(const string &n, const string &d)
    : D4Sequence(n, d), d_len(4), d_current(0), d_series_values(false) {}

TestD4Sequence::TestD4Sequence(const TestD4Sequence &rhs) : D4Sequence(rhs), TestCommon(rhs) { m_duplicate(rhs); }

TestD4Sequence::~TestD4Sequence() {}

TestD4Sequence &TestD4Sequence::operator=(const TestD4Sequence &rhs) {
    if (this == &rhs)
        return *this;
...
void TestD4Sequence::output_values(std::ostream &out) { print_val(out, "", false); }
...
void TestD4Sequence::set_series_values(bool sv) {
