...
static const char *states[] = {"start",
...
void DDXParser::set_state(DDXParser::ParseState state) { s.push(state); }

DDXParser::ParseState DDXParser::get_state() const { return s.top(); }

void DDXParser::pop_state() { s.pop(); }
...
/** Is an attribute present? Attribute names are always lower case.
    ...
    false otherwise. */
bool DDXParser::check_attribute(const string &attr) { return (attribute_table.find(attr) != attribute_table.end()); }
...
void DDXParser::process_variable(Type t, ParseState s, const xmlChar **attrs, int nb_attributes) {
    transfer_xml_attrs(attrs, nb_attributes);
...
inline bool DDXParser::is_attribute_or_alias(const char *name, const xmlChar **attrs, int nb_attributes) {
...
inline bool DDXParser::is_variable(const char *name, const xmlChar **attrs, int nb_attributes) {
...
void DDXParser::finish_variable(const char *tag, Type t, const char *expected) {
    ...
    if (t == dods_array_c && static_cast<Array *>(btp)->dimensions() == 0) {
    ...
    if (!(parent->is_vector_type() || parent->is_constructor_type())) {
        DDXParser::ddx_fatal_error(this, "Tried to add the array variable '%s' to a non-constructor type (%s %s).", tag,
                                   bt_stack.top()->type_name().c_str(), bt_stack.top()->name().c_str());
        delete btp;
        return;
    }
...
void DDXParser::ddx_sax2_start_element(void *p, const xmlChar *l, const xmlChar *prefix, const xmlChar *URI,
                                       int nb_namespaces, const xmlChar **namespaces, int nb_attributes,
                                       int /*nb_defaulted*/, const xmlChar **attributes) {
    DDXParser *parser = static_cast<DDXParser *>(p);
    const char *localname = (const char *)l;

    DBG2(cerr << "start element: " << localname << ", states: " << states[parser->get_state()]);
    ...
    case parser_start:
        if (strcmp(localname, "Dataset") == 0) {
            ...
            DDXParser::ddx_fatal_error(parser, "Expected response to start with a Dataset element; found '%s' instead.",
                                       localname);
        break;
    ...
    case inside_dataset:
        if (parser->is_attribute_or_alias(localname, attributes, nb_attributes))
            ...
            DDXParser::ddx_fatal_error(parser, "Expected an Attribute, Alias or variable element; found '%s' instead.",
                                       localname);
        break;
    ...
    case inside_attribute_container:
        if (parser->is_attribute_or_alias(localname, attributes, nb_attributes))
            ...
            DDXParser::ddx_fatal_error(parser, "Expected an Attribute or Alias element; found '%s' instead.",
                                       localname);
        break;
    ...
    case inside_attribute_value:
        ddx_fatal_error(parser, "Internal parser error; unexpected state, inside value while processing element '%s'.",
                        localname);
        break;
    ...
    case inside_alias:
        ddx_fatal_error(parser, "Internal parser error; unexpected state, inside alias while processing element '%s'.",
                        localname);
        break;
...
void DDXParser::ddx_sax2_end_element(void *p, const xmlChar *l, const xmlChar *prefix, const xmlChar *URI) {
    DDXParser *parser = static_cast<DDXParser *>(p);
    const char *localname = (const char *)l;

    DBG2(cerr << "End element " << localname << " (state " << states[parser->get_state()] << ")" << endl);
    ...
    case inside_dataset:
        if (strcmp(localname, "Dataset") == 0)
            ...
            DDXParser::ddx_fatal_error(parser, "Expected an end Dataset tag; found '%s' instead.", localname);
        break;
    ...
    case inside_attribute_container:
        if (strcmp(localname, "Attribute") == 0) {
            ...
            DDXParser::ddx_fatal_error(parser, "Expected an end Attribute tag; found '%s' instead.", localname);
        break;
    ...
    case inside_attribute_value:
        if (strcmp(localname, "value") == 0) {
            ...
            DDXParser::ddx_fatal_error(parser, "Expected an end value tag; found '%s' instead.", localname);
    ...
    case inside_other_xml_attribute: {
        if (strcmp(localname, "Attribute") == 0 && parser->root_ns == (const char *)URI) {
            ...
            DBGN(cerr << endl << "\t Popping the 'inside_other_xml_attribute' state" << endl);
            ...
            DBGN(cerr << endl
                      << "\t inside_other_xml_attribute: " << localname << ", depth: " << parser->other_xml_depth
                      << endl);
            if (parser->other_xml_depth == 0)
                DDXParser::ddx_fatal_error(parser, "Expected an OtherXML attribute to end! Instead I found '%s'",
                                           localname);
            parser->other_xml_depth--;
        ...
        if (is_simple_type(t)) {
            ...
            if (parent->is_vector_type() || parent->is_constructor_type()) {
                ...
                DDXParser::ddx_fatal_error(
                    parser, "Tried to add the simple-type variable '%s' to a non-constructor type (%s %s).", localname,
                    parser->bt_stack.top()->type_name().c_str(), parser->bt_stack.top()->name().c_str());
                delete btp;
            }
        } else {
            DDXParser::ddx_fatal_error(parser, "Expected an end tag for a simple type; found '%s' instead.", localname);
        }
        break;
    }
    ...
    case inside_dimension:
        if (strcmp(localname, "dimension") == 0)
            ...
            DDXParser::ddx_fatal_error(parser, "Expected an end dimension tag; found '%s' instead.", localname);
        break;
    ...
    case inside_blob_href:
        if (strcmp(localname, "blob") == 0 || strcmp(localname, "dataBLOB") == 0)
            ...
            DDXParser::ddx_fatal_error(parser, "Expected an end dataBLOB/blob tag; found '%s' instead.", localname);
        break;
 ...
 */
void DDXParser::ddx_ignoreable_whitespace(void *p, const xmlChar *ch, int len) {
    DDXParser *parser = static_cast<DDXParser *>(p);
...
void DDXParser::ddx_get_cdata(void *p, const xmlChar *value, int len) {
    ...
    default:
        DDXParser::ddx_fatal_error(parser, "Found a CData block but none are allowed by DAP.");
...
xmlEntityPtr DDXParser::ddx_get_entity(void *, const xmlChar *name) { return xmlGetPredefinedEntity(name); }
