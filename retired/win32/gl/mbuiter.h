...
static inline void mbuiter_multi_next(struct mbuiter_multi *iter) {
    ...
    if (is_basic(*iter->cur.ptr)) {
        ...
        iter->in_shift = true;
    with_shift:
        iter->cur.bytes = mbrtowc(&iter->cur.wc, iter->cur.ptr, strnlen1(iter->cur.ptr, MB_CUR_MAX), &iter->state);
...
static inline void mbuiter_multi_reloc(struct mbuiter_multi *iter, ptrdiff_t ptrdiff) { iter->cur.ptr += ptrdiff; }

/* Iteration macros.  */
typedef struct mbuiter_multi mbui_iterator_t;
#define mbui_init(iter, startptr)                                                                                      \
    ((iter).cur.ptr = (startptr), (iter).in_shift = false, memset(&(iter).state, '\0', sizeof(mbstate_t)),             \
     (iter).next_done = false)
#define mbui_avail(iter) (mbuiter_multi_next(&(iter)), !mb_isnul((iter).cur))
#define mbui_advance(iter) ((iter).cur.ptr += (iter).cur.bytes, (iter).next_done = false)
