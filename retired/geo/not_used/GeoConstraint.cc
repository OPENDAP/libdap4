...
public:
    ...
    bool operator()(const string &prefix) { return s.find(prefix) == 0; }
};
...
GeoConstraint::Notation GeoConstraint::categorize_notation(const double left, const double right) const {
...
void GeoConstraint::transform_constraint_to_pos_notation(double &left, double &right) const {
    if (left < 0)
        left += 360;
...
bool GeoConstraint::is_bounding_box_valid(const double left, const double top, const double right,
                                          const double bottom) const {
    if ((left < d_lon[0] && right < d_lon[0]) || (left > d_lon[d_lon_length - 1] && right > d_lon[d_lon_length - 1]))
        return false;
...
/** Scan from the left to the right, and the right to the left, looking
    ...
    index. */
void GeoConstraint::find_longitude_indeces(double left, double right, int &longitude_index_left,
...
/** Scan from the top to the bottom, and the bottom to the top, looking
    ...
    index. */
void GeoConstraint::find_latitude_indeces(double top, double bottom, LatitudeSense sense, int &latitude_index_top,
    ...
    if (sense == normal) {
        ...
        if (d_lat[j] == bottom)
            ...
            latitude_index_bottom = (j + 1) < d_lat_length - 1 ? j + 1 : d_lat_length - 1;
    } else {
        i = d_lat_length - 1;
...
static void swap_vector_ends(char *dest, char *src, int len, int index, int elem_sz) {
...
template <class T>
static void transpose(std::vector<std::vector<T>> a, std::vector<std::vector<T>> b, int width, int height) {
...
static int count_size_except_latitude_and_longitude(Array &a) {
...
void GeoConstraint::flip_latitude_within_array(Array &a, int lat_length, int lon_length) {
    if (!d_array_data) {
        a.read();
    ...
    for (int i = 0; i < size; ++i) {
        ...
        while (s_lat > -1)
            memcpy(tmp_data.data() + offset + (lat++ * lon_size), d_array_data + offset + (s_lat-- * lon_size),
                   lon_size);
    }
...
static int count_dimensions_except_longitude(Array &a) {
    int size = 1;
...
void GeoConstraint::reorder_data_longitude_axis(Array &a, Array::Dim_iter lon_dim) {
    ...
    if (!is_longitude_rightmost())
        throw Error("This grid does not have Longitude as its rightmost dimension, the geogrid()\ndoes not support "
                    "constraints that wrap around the edges of this type of grid.");

    DBG(cerr << "Constraint for the left half: " << get_longitude_index_left() << ", " << get_lon_length() - 1 << endl);

    // Build a constraint for the left part and get those values
    a.add_constraint(lon_dim, get_longitude_index_left(), 1, get_lon_length() - 1);
    a.set_read_p(false);
    a.read();
    ...
    DBG(cerr << "Constraint for the right half: " << 0 << ", " << get_longitude_index_right() << endl);
    ...
    DBG2(cerr << "elem_size: " << elem_size << "; left & right size: " << left_row_size << ", " << right_row_size
              << endl);
    ...
    for (int i = 0; i < rows_to_copy; ++i) {
        ...
        memcpy(d_array_data + (total_bytes_per_row * i), left_data + (left_row_size * i), left_row_size);

        DBG(cerr << "right memcpy: " << *(float *)(right_data + (right_row_size * i)) << endl);

        memcpy(d_array_data + (total_bytes_per_row * i) + left_row_size, right_data + (right_row_size * i),
               right_row_size);
    }
...
GeoConstraint::GeoConstraint()
    : d_array_data(0), d_array_data_size(0), d_lat(0), d_lon(0), d_lat_length(0), d_lon_length(0),
      d_latitude_index_top(0), d_latitude_index_bottom(0), d_longitude_index_left(0), d_longitude_index_right(0),
      d_bounding_box_set(false), d_longitude_rightmost(false), d_longitude_notation(unknown_notation),
...
void GeoConstraint::set_bounding_box(double top, double left, double bottom, double right) {
    ...
    if (!is_bounding_box_valid(left, top, right, bottom))
        throw Error("The bounding box does not intersect any data within this Grid or Array. The\ngeographical extent "
                    "of these data are from latitude " +
                    double_to_string(d_lat[0]) + " to " + double_to_string(d_lat[d_lat_length - 1]) +
                    "\nand longitude " + double_to_string(d_lon[0]) + " to " +
                    double_to_string(d_lon[d_lon_length - 1]) + " while the bounding box provided was latitude " +
                    double_to_string(top) + " to " + double_to_string(bottom) + "\nand longitude " +
                    double_to_string(left) + " to " + double_to_string(right));
    ...
    find_latitude_indeces(top, bottom, d_latitude_sense, d_latitude_index_top, d_latitude_index_bottom);
    ...
    DBG(cerr << "Bounding box (tlbr): " << d_latitude_index_top << ", " << d_longitude_index_left << ", "
             << d_latitude_index_bottom << ", " << d_longitude_index_right << endl);
