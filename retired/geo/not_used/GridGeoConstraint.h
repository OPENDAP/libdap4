...
extern bool unit_or_name_match(set<string> units, set<string> names, const string &var_units, const string &var_name);
...
private:
    // Specific to a Grid
    libdap::Grid *d_grid; //< Constrain this Grid (weak pointer)

    libdap::Array *d_latitude;  //< A (weak) pointer to the Grid's latitude map
    libdap::Array *d_longitude; //< A (weak) pointer to the Grid's longitude map
...
public:
    ...
    virtual libdap::Grid *get_constrained_grid() const { return d_grid; }
};
