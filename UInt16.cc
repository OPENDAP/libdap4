...
UInt16::UInt16(const UInt16 &copy_from) : BaseType(copy_from) { d_buf = copy_from.d_buf; }

BaseType *UInt16::ptr_duplicate() { return new UInt16(*this); }

UInt16 &UInt16::operator=(const UInt16 &rhs) {
    if (this == &rhs)
        return *this;
...
bool UInt16::serialize(ConstraintEvaluator &eval, DDS &dds, Marshaller &m, bool ce_eval) {
...
bool UInt16::deserialize(UnMarshaller &um, DDS *, bool) {
    um.get_uint16(d_buf);

    return false;
}

void UInt16::compute_checksum(Crc32 &checksum) { checksum.AddData(reinterpret_cast<uint8_t *>(&d_buf), sizeof(d_buf)); }
 ...
 */
void UInt16::serialize(D4StreamMarshaller &m, DMR &, /*ConstraintEvaluator &,*/ bool) {
    if (!read_p())
...
void UInt16::deserialize(D4StreamUnMarshaller &um, DMR &) { um.get_uint16(d_buf); }

unsigned int UInt16::val2buf(void *val, bool) {
    // Jose Garcia
...
unsigned int UInt16::buf2val(void **val) {
    // Jose Garcia
...
dods_uint16 UInt16::value() const { return d_buf; }

bool UInt16::set_value(dods_uint16 i) {
    d_buf = i;
    set_read_p(true);
...
void UInt16::print_val(FILE *out, string space, bool print_decl_p, bool is_root_grp) {
    ostringstream oss;
    print_val(oss, space, print_decl_p, is_root_grp);
    fwrite(oss.str().data(), sizeof(char), oss.str().length(), out);
}

void UInt16::print_val(ostream &out, string space, bool print_decl_p, bool is_root_grp) {
    if (print_decl_p) {
...
bool UInt16::ops(BaseType *b, int op) {
 ...
 * @return void
 */
void UInt16::dump(ostream &strm) const {
    strm << DapIndent::LMarg << "UInt16::dump - (" << (void *)this << ")" << endl;
    DapIndent::Indent();
    BaseType::dump(strm);
